...
void CACHE_REPLACEMENT_STATE::InitReplacementState()
{
    ...
    hotclock = new clock*[numsets];
    coldclock = new clock*[numsets];
    hothand = new int[numsets];
    coldhand = new int[numsets];
    ...
    for(UINT32 setIndex=0; setIndex<numsets; setIndex++) 
    {
        ...
        hotclock[setIndex] = new clock[assoc];
        coldclock[setIndex] = new clock[assoc >> 1];
        hothand[setIndex] = 0;
        coldhand[setIndex] = 0;
        ...
        for(UINT32 way=0; way<assoc; way++) 
        {
            ...
            hotclock[setIndex][way].ref = 0;
            hotclock[setIndex][way].idx = way;
            repl[setIndex][way].hotpos = way;
            if (way < (assoc >> 1)) {
	            coldclock[setIndex][way].ref = 0;
    	        coldclock[setIndex][way].idx = way;
    	        repl[setIndex][way].coldpos = (way < (assoc >> 1)) ? way : -1;
    	    }
        }
    }
    ...
    UINT32 tid, Addr_t PC, UINT32 accessType, bool cacheHit )
{
    ...
    else if( replPolicy == CRC_REPL_CONTESTANT )
    {
        ...
        UpdateCLIRS(setIndex, updateWayID);
    }
